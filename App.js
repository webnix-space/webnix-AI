...
// ^ TEMPORARILY DISABLED to isolate launch crash — stubbed below
...
try {
  ...
  console.log("[QVAC] QWEN3_600M_INST_Q4:", QWEN3_600M_INST_Q4);
} catch (e) {
...
export default function App() {
  ...
  // Voice — TEMPORARILY STUBBED to isolate launch crash (expo-audio disabled)
  const audioRecorder = null;
  const recorderState = { isRecording: false };
  ...
  useEffect(() => {
    ...
    (async () => {
      const savedModelId = await load(STORAGE_KEYS.MODEL, "llama");
  ...
  // ── Voice recording — TEMPORARILY STUBBED to isolate launch crash ──
  const startRecording = async () => {
    Alert.alert("Voice disabled", "Voice input is temporarily disabled while we debug a build issue. Type your message instead.");
  };
