...
export default function App() {
  ...
  const updateChat = useCallback((updated) => {
    const clean = updated.filter(m => !m.streaming);
    setChat(updated);
  ...
  // ── New chat ───────────────────────────────────────────────
  const newChat = () => {
    Alert.alert("New Chat", "Clear conversation history?", [
      { text: "Cancel", style: "cancel" },
      { text: "Clear", style: "destructive", onPress: () => { updateChat([]); setTps(null); log("Chat cleared"); } },
    ]);
  };
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      {/* Header — fixed overlap + badge placement */}
      <View style={s.header}>
      ...
      {tab === TABS.ASK && (
        ...
        >
          ...
          {/* Input row — fixed keyboard hiding */}
          <View style={s.inputRow}>
            <TextInput
              ...
              blurOnSubmit={false}
            />
            {/* Mic button — triggers keyboard mic on Android */}
            <TouchableOpacity
              style={s.micBtn}
              onPress={() => {
...
const s = StyleSheet.create({
  ...
  header:        { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingTop: 10, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: C.border },
  headerLeft:    { flex: 1 },
